<?php
// Archivos Excel de inventario por sucursal (tabla cierre_inventario), subidos desde el tab Inventario.
...
if ($method === 'GET') {
    ...
    $stmt = db()->prepare(
        "SELECT sucursal, headers, filas, updated_at FROM cierre_inventario WHERE mes_key = ?"
    );
    $stmt->execute([$mesKey]);
    $rows = $stmt->fetchAll();
    $out = [];
...
if ($method === 'POST') {
    ...
    $stmt = db()->prepare(
        "INSERT INTO cierre_inventario (mes_key, sucursal, headers, filas)
         VALUES (?,?,?,?)
